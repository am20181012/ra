...
<body>
    ...
    <form action="">
        ...
        <div id="kategorije_put">
            <input type="text" name="kategorija_id_put" id="kategorija_id_put" placeholder="Unesite id kategorije koju zelite da izmenite">
            <br>
            <input type="text" name="kategorija_naziv_put" id="kategorija_naziv_put" placeholder="Unesite novi naziv kategorije">
        </div>

        <div id="novosti_put">
            <input type="text" name="novosti_id_put" id="novosti_id_put" placeholder="Unesite id novosti koju zelite da izmenite">
            <br>
            ...
            switch($("input[name=http_zahtev]:checked")[0].id){
                ...
                case "get":
                    if(tabela=="radio_novosti"){
                        $.getJSON("http://localhost/rest/novosti.json", function(data){
                            document.getElementById("get_odgovor").innerHTML = JSON.stringify(data,null,2);
                        });
                    }else{
                        $.getJSON("http://localhost/rest/kategorije.json", function(data){
                            document.getElementById("get_odgovor").innerHTML = JSON.stringify(data,null,2);
                        });
                    }
                    break;
                ...
                case "post":
                    if(tabela=="radio_novosti"){
                        ...
                        console.log(values);
                        $.post("http://localhost/rest/novosti", JSON.stringify(values), function(data){
                        ...
                        var values = {
                            ...
                            console.log(values);
                            $.post("http://localhost/rest/kategorije", JSON.stringify(values), function(data){
                                alert("Odgovor od servera> " + data['poruka']);
                            });
                    }
                    break;
                ...
                case "put":
                    if(tabela=="radio_novosti"){
                        ...
                        var id = parseInt($("input[name=novosti_id_put]").val());
                        console.log(values);
                        $.ajax({
                            url: "http://localhost/rest/novosti/" + id,
                        ...
                        var values = {
                            "kategorija_id" : parseInt($("#kategorija_id_put").val()),
                            "kategorija": $("input[name=kategorija_naziv_put]").val()
                            };
                        console.log(values);
                        $.ajax({
                            url: "http://localhost/rest/kategorije/" + values.kategorija_id,
                            data: JSON.stringify(values),
                            type:"PUT",
                            success: function(data){
                                alert("Odgovor od servera> " + data['poruka']);
                ...
                case "delete":
                    ...
                    if(tabela=="radio_novosti"){
                        ...
                        $.ajax({
                            url: "http://localhost/rest/kategorije/" + id,
                            type:"DELETE",
                            success: function(data){
                                alert("Odgovor od servera> " + data['poruka']);
                            }
